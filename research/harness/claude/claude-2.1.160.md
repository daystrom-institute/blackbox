...
topic:
  - harness
  - claude
brief: "Point-in-time research snapshot for Claude Code 2.1.160: the one provenance record for this subject version, plus the per-axis checklist coordinating research across the ten harness axes. Seeded with well-defined elements backfilled from the existing bro-harness binary mines and from direct observation of the running 2.1.160 harness; the deeper mine is still open. First snapshot for the claude subject — no predecessor."
---
...
> **This snapshot is seeded, not fully mined.** Each leaf carries well-defined
> elements backfilled two ways: (a) from the existing bro-harness binary mines in
> `design/bro-harness/` (the canonicalization work that already read 2.1.160),
> and (b) from **direct observation of the running 2.1.160 harness** (this very
> session is Claude Code 2.1.160 — its tool docs, system reminders, skill list,
> and deferred-tool mechanics are observed first-hand, which is the strongest
> provenance for behavioral claims). The deeper string-mine of the binary is left
> open per leaf.
...
- **Binary:** `~/.local/share/claude/versions/2.1.160` — ELF 64-bit LSB
  executable, x86_64, ~231.6 MB (Bun-compiled single file; the JS bundle and its
  string literals are embedded as plaintext).
- **Host:** Linux 6.12.x x86_64 (Manjaro).
...
| Hooks | [claude-hooks](claude-hooks.md) | researching | mixed | blocking pre-tool + output-as-feedback known; event catalogue open |
...
- String-mine the binary for the items each leaf marks **Open** (per-flag header
  values, compaction threshold call graph, hook event catalogue).
