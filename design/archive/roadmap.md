...
stored via the generic `bbox_link` graph surface (see `design/commit-work-provenance.md`)
...
- **Missing**: `file_path` doesn't exist → render as plain text with
  `[missing: design/roadmap.md]`. The file was renamed or deleted.
...
defined in `design/commit-work-provenance.md`. Roadmap edges are not a
