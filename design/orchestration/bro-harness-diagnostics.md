...
> Latency figures below were measured this session on this workspace.
...
- **Transport — exists.** `LspSessionManager` pools warm sessions keyed by
  `(project_root, Language)`, resolves binaries from `LspConfig`
  (`rust_analyzer_bin`, `jdtls_bin`, `roslyn_lsp_bin`), evicts on idle, and fails
  closed (`error.lsp_unavailable`, per RX-V3). Adding a language is a 4-site
  registration (`Language` enum + `LspConfig` bin + `launch_argv` +
...
### Daemon ↔ harness boundary

Resolved by the cluster convention (*async/temporal sessions are harness-owned;
MCP tools stay synchronous unary*):

- **Pull diagnostics is unary** → expose `bbox_code_diagnostics(file, version)`
  as a synchronous-unary daemon MCP tool backed by the *existing warm*
  `LspSessionManager`. No duplicate harness-local language server, no convention
  violation. The per-language classification adapter lives here.
- **window=0 is harness loop policy**, not a harness-owned session: the harness
  calls the unary tool synchronously in its `file_edit`/`file_write` path and
  riders the result (same append mechanism as `bound.rs`). After writing to disk,
  the daemon session `didChange`s from on-disk content and pulls; version = file
  blob hash.
- **Ensemble falls out for free:** sessions key on `(project_root, Language)`, and
  a drone's worktree *is* its `project_root` — each lane gets an isolated warm
  session with no extra work. The existing key was already ensemble-correct.
- **Truth tier is partly non-LSP:** for Rust it is out-of-band cargo
  (`--all-features`/clippy/udeps) because the LSP server runs one default-feature
  flycheck config; for C# the LSP server *is* the truth tier. So the provider
...
LspSessionManager (BUILT, nav-only) ──┬─→ extract diagnostics (stop discarding)
                                      ├─→ didChange document-sync (net-new)
                                      └─→ pull diagnostics where supported (net-new)
classification adapter per Language (NET-NEW) ──→ universal diagnostic envelope
bbox_code_diagnostics unary MCP tool (NET-NEW) ──→ harness file_edit window=0 call
truth_check(scope) per Language (NET-NEW, partly non-LSP) ──→ ownership-transfer gate
supervision AlertKind::loop (BUILT) ──→ escalation signal (build-on-smell spiral)
```

**Built / reusable:** `LspSessionManager` (+ `(project_root, Language)` keying,
idle eviction, fail-closed), the `publishDiagnostics` drain, the `side` spine,
the `bound.rs` rider, the hooks injection seam.
**Net-new:** diagnostic payload extraction, `didChange` document-sync, pull
diagnostics, the per-language classification adapter, the diff-with-stable-identity
differ, the `bbox_code_diagnostics` unary tool, the per-language `truth_check`.
