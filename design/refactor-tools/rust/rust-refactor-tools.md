---
title: "Rust Refactor Tools"
kind: design-hub
corpus: blackbox-design
topic:
  - refactor-tools
  - rust
tags:
  - refactor-tools
  - rust
brief: "Hub for Rust refactor expansion, Rust refactor atoms, v2 invariants, and current gap inventory."
---

# Rust Refactor Tools

Rust refactor work centers on closing the Java deep-analysis gap while keeping
LSP-backed and compiler-backed operations explicit about their semantic grade.

## Docs

- [Rust Refactor Expansion](refactor-rust-expansion.md)
- [Rust Refactor Expansion - Implementation Skeleton](refactor-rust-expansion-impl.md)
- [Rust Refactor v2 Invariants](refactor-rust-v2-invariants.md)
- [Rust Refactor Atoms - Batch 2](rust-refactor-atoms-batch2.md)
- [Rust Refactor Gap Inventory](rust-refactor-gap-inventory.md)

## Crosscuts

- [Refactor Tools](../refactor-tools.md)
- [Refactor Agents](../refactor-agents.md)
- [Atom Capability Runtime](../../orchestration/atoms/atom-capability-runtime.md)
