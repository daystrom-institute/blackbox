---
title: "Config and Artifact Locality Implementation Plan"
kind: design
lifecycle: archived
...
Companion to `design/operations/config-artifacts/config-and-artifact-locality.md`. This document is the
...
- Config loader: `figment`, not `config-rs`, because provider stacking maps
  directly to `default -> file -> env -> flag` (`design/operations/config-artifacts/config-and-artifact-locality.md:202-205`).
...
- File watching: `notify = "8"` plus `notify-debouncer-full = "0.6"` as the
  concrete debounced watcher stack (`design/operations/config-artifacts/config-and-artifact-locality.md:351-352`).
...
1. Confirm no in-flight branch already creates `src/config.rs`,
   `src/secrets.rs`, or `design/operations/config-artifacts/config-and-artifact-locality-impl.md`.
...
2. Add `bbox_project_init`.
   ...
   - Do not trigger reindex. The design explicitly says init scaffolds only
     (`design/operations/config-artifacts/config-and-artifact-locality.md:330`).
