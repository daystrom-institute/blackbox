---
title: "Corpus"
kind: design-hub
corpus: blackbox-design
topic:
  - corpus
brief: "Hub for Blackbox corpus, knowledge, provenance, storage, code navigation, and Badgey designs."
---

# Corpus

This hub groups the designs for Blackbox as a searchable, provenance-aware
working memory: indexed project files, transcripts, knowledge, notes, graph
edges, storage, and corpus-facing assistants.

## Core Corpus

- [Agentic Corpus](agentic-corpus/agentic-corpus-platform.md)
- [Design Graph](agentic-corpus/design-graph.md)
- [Gap-to-Campaign Migration Plan](agentic-corpus/gap-campaign-migration.md)
- [Reflective Project Graph](agentic-corpus/reflective-project-graph.md)
- [Badgey](badgey.md)
- [Badgey - Implementation Skeleton](badgey-impl.md)
- [Commit Work Provenance](commit-work-provenance.md)
- [Roadmap](roadmap.md)

## Knowledge And Memory

- [Knowledge And Memory](knowledge/knowledge-and-memory.md)
- [Knowledge Store](knowledge/knowledge-store.md)
- [Note-backed Gap Log](knowledge/note-backed-gap-log.md)
- [System Memories Runtime Loading](knowledge/system-memories-runtime-loading.md)

## Retrieval And Storage

- [Locate-Information Coherence Path](agentic-corpus/locate-information-coherence.md)
- [Retrieval Eval Harness](agentic-corpus/retrieval-eval-harness.md)
- [Code Navigation](code-navigation/code-navigation.md)
- [Storage And Performance](storage/storage-and-performance.md)

## Crosscuts

- [Orchestration](../orchestration/orchestration.md)
- [Surfaces](../surfaces/surfaces.md)
- [Operations](../operations/operations.md)
