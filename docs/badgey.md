...
Long-lived corpus consultant. Given a project scope, Badgey traces
provenance, walks the typed entity graph, explains why decisions exist,
investigates threads and sessions, and distills recurring transcript
patterns into proposed durable artifacts (knowledge entries, brofiles,
packets, workflows, agents) — every claim cited against round-trippable
entity refs.
...
with a work-item thread of record. Turns are serialized per instance —
...
Install the example artifacts from `examples/badgey/`:
