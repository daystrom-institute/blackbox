...
| [Whiteboard](whiteboard/whiteboard-example.md) | ADR-tagged issue webhook | Three specialist agents (security / performance / design) post stances blind to a phaser-style whiteboard, validate each other's claims against evidence, exchange challenges and rebuttals across gated response rounds (loop on `unresolved_challenges` with an agree-to-disagree ceiling), then vote informed by the exchange. Facilitator synthesizes an ADR markdown file (surviving disagreements included), opens a PR, auto-merges on consensus. Exercises the engine's whiteboard primitive, multi-round durable ensemble dispatch, deliberative gate loops, per-actor dispatch timeouts, and human-in-the-loop via the same `whiteboard_*` MCP surface specialists use. |
