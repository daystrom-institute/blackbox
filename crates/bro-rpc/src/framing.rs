...
impl<T> FramedIo<T> {
    ...
    pub(crate) fn into_parts(self) -> (T, usize) {
        (self.io, self.max_frame_bytes)
    }
}
...
pub(crate) async fn read_json_frame<R: AsyncRead + Unpin, V: DeserializeOwned>(
    ...
    read_json_frame_with_len(reader, max_frame_bytes)
        .await
        .map(|(value, _)| value)
}

pub(crate) async fn read_json_frame_with_len<R: AsyncRead + Unpin, V: DeserializeOwned>(
    reader: &mut R,
    max_frame_bytes: usize,
) -> Result<(V, usize), RpcError> {
    ...
    Ok((serde_json::from_slice(&bytes)?, len))
}
