...
fn snapshot_transcript_styled_multi_width() {
    ...
    for width in [40usize, 80, 120] {
        let lines = render_transcript(
            &items,
            "this is a fairly long initial steer prompt that should word-wrap across several lines at the narrow forty-column width",
            &["a queued turn"],
            width,
        );
...
fn inline_stable_end_active_turn_holds_back_all_items_until_result() {
    ...
    assert_eq!(
        inline_stable_end(&[TranscriptItem::AssistantText("hi".into()),], true),
        0
    );
    assert_eq!(
        inline_stable_end(
...
fn render_markdown_loose_bullet_marker_stitched_to_content() {
    ...
    let rendered: Vec<String> = render_markdown("- item with **bold**\n\n  paragraph under item\n")
        .iter()
        .map(line_text)
        .collect();
    assert!(
...
fn compact_tool_call_line_uses_positional_single_arg() {
    let line = compact_tool_call_line("smart_read", r#"{"path":"src/knowledge.rs"}"#, 100).unwrap();
...
fn control_write_error_classes_are_detected() {
    ...
    assert!(err_is_not_running("task abc is Completed, not running"));
...
fn compact_tool_call_line_quotes_shell_commands() {
    let line = compact_tool_call_line("shell_run", r#"{"cmd":"cargo test --lib"}"#, 100).unwrap();
...
#[test]
fn apply_patch_tool_call_renders_source_as_diff_block() {
    let items = vec![TranscriptItem::ToolCall {
...
#[test]
fn exec_tool_call_renders_source_as_js_code_block() {
    let items = vec![TranscriptItem::ToolCall {
        name: "exec".into(),
        args: serde_json::json!({
            "source": "const value = await tools.echo({ n: 1 });\ntext(JSON.stringify(value));"
        })
        .to_string(),
    }];
    let rendered: Vec<String> = render_transcript(&items, "", &[], 100)
        ...
        .map(line_text)
        .collect();
    assert_eq!(rendered.first().map(String::as_str), Some("▸ exec(js)"));
    assert!(rendered.iter().any(|line| line == "┌─ js"), "{rendered:?}");
    assert!(
        rendered.iter().any(|line| line.contains("const value")),
        "{rendered:?}"
    );
    assert!(
        rendered
            .iter()
            .any(|line| line.contains("text(JSON.stringify(value));")),
        "{rendered:?}"
    );
}
...
fn roster_model_label_falls_back_to_provider_default() {
    ...
    assert_ne!(
        m, "—",
        "default must be the provider's catalog model, not the em-dash"
    );
}
...
fn prune_status_reports_unmerged_branch_kept() {
    let msg = prune_status_message(2, 0, 1, 0, 0, &["bro-fleet/test-branch".to_string()]);
    assert!(
...
fn terminal_agent_single_enter_submits() {
    ...
    assert!(
        app.input.is_empty(),
        "first Enter must submit (resume path takes the input)"
    );
    assert!(
...
fn footer_status_flash_precedes_mismatch_banner() {
    ...
    app.status = Some("Ctrl+K again to prune 3".to_string());
...
fn zoom_rename_slash_without_arg_shows_usage() {
    ...
    assert!(
        app.status
            .as_deref()
            .is_some_and(|s| s.contains("usage: /rename")),
...
fn install_resume_failure_keeps_row_and_restores_turn() {
    ...
    assert_eq!(
        app.input, "the lost turn",
        "turn must return to the composer"
    );
    assert!(
        !app.resuming.contains("resume-fail-1"),
        "resume gate must clear"
    );
    assert!(
        app.pending_cockpit_lines
            .first()
