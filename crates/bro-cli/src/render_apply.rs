    ...
    for item in &plan.items {
        match item {
            RenderPlanItemV1::WholeFile {
                ...
                match apply_whole_file(&target, contents, args.check)? {
...
/// blackbox-generated.
fn apply_whole_file(target: &Path, contents: &str, check: bool) -> Result<ApplyOutcome> {
...
mod tests {
    ...
    fn whole_file_writes_generated_and_fresh_targets() {
        ...
        assert!(matches!(
            apply_whole_file(&target, body, false).unwrap(),
        ...
        assert!(matches!(
            apply_whole_file(&target, body2, false).unwrap(),
