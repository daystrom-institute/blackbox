...
use bbox_edge_index::edge_index::{Edge, exact_edge};
...
pub struct GraphGeneration {
    ...
    pub projected_edges: Vec<Edge>,
    pub fingerprint: String,
    pub source_root: PathBuf,
}

impl GraphGeneration {
    pub fn vertex_ref(&self, vertex_id: &str) -> EntityRef {
        EntityRef::ProjectGraphVertex {
            scope_id: self.key.scope_id.clone(),
            graph_id: self.key.graph_id.clone(),
            vertex_id: vertex_id.to_string(),
        }
    }

    pub fn known_refs(&self) -> Vec<EntityRef> {
        self.vertices.keys().map(|id| self.vertex_ref(id)).collect()
    }
...
pub(crate) fn project_generation(
    ...
    let projected_edges = all_edges
        ...
        .map(|edge| {
            ...
            projected.metadata = edge
                .properties
                .iter()
                .map(|(name, value)| (name.clone(), property_value_string(value)))
                .collect();
            projected
        })
        .collect();
...
impl ProjectGraphCatalog {
    ...
    pub fn forward_edges(&self, entity: &EntityRef, include_local: bool) -> Vec<Edge> {
        self.edges_for(entity, include_local, true)
    }

    pub fn reverse_edges(&self, entity: &EntityRef, include_local: bool) -> Vec<Edge> {
        self.edges_for(entity, include_local, false)
    }
