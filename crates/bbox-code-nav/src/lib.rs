...
use bbox_chunker::code::{language_for_path, parser_for_language, ts_language_for_name};
...
pub fn check_project_dir_registered(
...
pub fn code_symbols_live(
    ...
    for entry in walker.filter_map(|entry| entry.ok()) {
        ...
        let status_json = match bbox_refactor::status(&status_params) {
            Ok(json) => json,
            Err(err) => {
                if errors.len() < 20 {
                    errors.push(CodeSymbolSearchError {
...
fn is_containing_symbol_kind(kind: &str) -> bool {
    (bbox_chunker::code::is_symbol_node(kind) || kind == "impl_item")
        && !is_root_kind(kind)
