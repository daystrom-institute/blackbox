...
- The same store root owns only the last authenticated provenance export
  receipt per catalog project. Receipt replacement is atomic, exact retries
  are no-ops, and server-side catalog authorization plus current-plan matching
  happen before this store is called.
