...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use to approve or reject existing unverified entries. Review controls render eligibility; it does not import rendered-file edits. For approve/reject, pass project to select a checkout-owner project when IDs overlap or unrelated publications are unavailable; the selected project must contain the entry, with no fallback to another owner. Omit project for list, global entries, or local-store entries. Unscoped mutations refuse when a unique owner cannot be established. The default list returns JSON with total, bounded content previews, and a content-bound next_action; use that action verbatim for the next page or a row detail_action for an exact record. Concatenate body.text pages, then parse the complete JSON. Queue or record changes invalidate cursors; restart with action=\"list\" or the row detail_action. See `sm-render-lifecycle` via `bbox_knowledge` for the full lifecycle.",
        example: None,
    },
    ToolDoc {
        name: "bbox_bootstrap",
