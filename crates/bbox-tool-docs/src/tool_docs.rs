...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        summary: "Manage the operator-directed roadmap; never use it to defer requested work. Reads return bounded summaries. list/search use limit/offset; next uses n/offset (max 100). detail=body and cursor recover exact JSON, including markdown as JSON strings. Paging controls are rejected for mutations.",
