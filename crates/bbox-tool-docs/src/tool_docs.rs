...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use when a checkout is going away or should stop being a local source for the project. Detach keeps every stored row: it is not deletion, and re-attaching later restores the local source. Catalog detachment can complete before auxiliary watcher/census cleanup fails; status=partial preserves the committed detach with cleanup outcomes.",
        example: None,
    },
    ToolDoc {
    ...
    ToolDoc {
        ...
        category: ToolCategory::Packets,
        summary: "Inspect MCP surface routing compactly. Replay pages visible tools and reports policy counts; describe pages rule summaries; list pages installed packets. Use detail=policy for exact allow/disallow patterns. body_limit/cursor reads all projected rows as exact JSON; omit limit/offset.",
    ...
    ToolDoc {
        ...
        summary: "Manage the operator-directed roadmap. Reads default to bounded summaries; list/search use limit (default 20, max 100) and offset with next_offset; next ranks n (default 5, max 100) and also accepts offset. detail=body returns exact JSON pages for get/list/search/next/render/default_template; concatenate body.text then parse JSON (render/template are JSON strings). Replay cursor with unchanged selectors; changes refuse continuation. Manage an operator-directed prospective work tracker for designed-but-not-implemented features, refactors, explorations, tech debt, and risks. Roadmap interactions are performed only at the express direction of the operator; never use the roadmap to defer, postpone, or avoid requested implementation work. Inbox is reactive; threads are active work; knowledge is atemporal. Status lifecycle: proposed → accepted → delivered (shipped) or rejected; accepted → deferred → accepted.",
