...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use before an advance to read its compare-and-swap tokens, and when diagnosing why published knowledge or gaps are unavailable, degraded to a prior generation, or refusing to advance. The health object answers the adjacent questions: which attachment the pointer names and whether it is still attached, which capabilities an attachment actually records, whether a checkout's overlay is fresh, and whether a watcher is registered.",
        example: None,
    },
    // ── Knowledge ────────────────────────────────────────────────────
    ToolDoc {
