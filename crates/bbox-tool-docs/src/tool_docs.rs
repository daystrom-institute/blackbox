...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Save templates, instantiate teams, inspect roster, or tear teams down. New advisor arguments are rejected. Legacy advisor settings/history remain readable and are marked execution=retired in summaries; create and waits never execute them. Dispatch any reviewer explicitly with bro_exec/bro_resume. list/list_templates/roster page at default 20, maximum 100; next_offset continues with the same exact name/project filters. get/get_template return exact stored JSON: concatenate body.text pages using cursor=body.next_cursor, then parse; source changes invalidate cursors. Template scope accepts global or project; each scope reads exactly one store and never falls back. In catalog mode project_dir selects a catalog project (id, alias or attached checkout path; it never grants daemon filesystem access). Project list_templates/get_template read only that project's accepted publication and name its generation and commit in `source`. Project save_template/delete_template queue a guarded edit of `.bro/teamplates/<name>.json` for the project's checkout owner and return state=queued with `mutation` (mutation_id, landing path, precondition, predecessor, accepted_generation, next_step); consecutive edits chain on the queued ones. The edit takes effect for reads, member validation and create only after the owner commits and publishes it. A project with no accepted publication, a publication without the configuration lane, invalid configuration, or a selector that names no catalog project refuse by name (error.project_config_publication_unavailable, error.project_config_lane_unsupported, error.project_config_invalid, error.project_config_project_unknown) with no global fallback. create and save_template member validation resolve the selected project's accepted templates/brofiles first, then global ones only when the accepted view proves the name absent; the create receipt attributes the source (templateScope, templateSource). Bridge mode reads and writes the project's local .bro/teamplates directory (absolute project_dir). Expanded membership must be 1..256; counts outside this range refuse before allocation or persistence. Team members resume existing sessions on later broadcasts; dissolve/recreate when validating new brofile context or provider-default suppression. See `sm-brofile-context` via `bbox_knowledge`. Before `save_template` or `create`, list existing objects first to avoid duplicates. Dissolve ad hoc teams you created after their work is terminal; do not dissolve another operator's team unless instructed. See `sm-create-etiquette` via `bbox_knowledge` for dedupe hygiene. Wrong-action mutation fields refuse. Dissolve caps cancellation at 64 tasks/16 KiB encoded IDs before effects and reports cancellation_requested plus skipped terminal/missing history and removal status. Cancellation failure retains the team.",
        example: Some(
