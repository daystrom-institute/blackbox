...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        category: ToolCategory::ProjectGraphs,
        summary: "Describe one visible project graph. detail=summary (default) stays compact: identity, generation, authority plane, retrieval state, and schema counts without the schema body. detail=schema or detail=descriptor recovers the exact JSON in bounded body pages.",
        when_to_use: "Read generation identity and word-index participation without pulling the schema. The summary keeps both count families (reflected vertex_count/edge_count versus authored_* from the jsonl sources) plus the retrieval block: policy flags, excluded vertex types, indexed vertex count and indexed generation versus accepted generation, and embed participation, so a graph missing from search is diagnosed before touching the schema. For exact bytes, pass detail=schema or detail=descriptor; body pages are 4..4096 UTF-8 bytes (body_limit, default 4096), continue with cursor=body.next_cursor, and concatenate body.text across pages before parsing. Cursors are content-bound to the project, selection, graph id, generation, and detail kind; any change refuses with a restart-without-cursor error. An entry accepted invalid carries schema:null in the summary and refuses detail reads with error.graph_payload_unavailable; use bbox_project_graph_validate for its diagnostics. Under provisional=all one graph id can match multiple live generations; summary lists them and exact reads refuse with error.project_graph_ambiguous until you narrow to published or own.",
        example: None,
    },
    ToolDoc {
        ...
        when_to_use: "Inspect kernel diagnostics. Reports the same three sources as list: published, provisional, and connector. Continue summary pages with next_error_offset plus expected_error_stamp; a changed error set refuses with error.graph_errors_changed and you restart at error_offset 0, while nonzero offsets without a stamp are refused outright. detail=errors pages the complete array through the shared body reader: body pages are 4..4096 UTF-8 bytes (body_limit, default 4096), continue with cursor=body.next_cursor, and concatenate body.text before parsing; cursors refuse any graph, selection, or content change. A valid graph reports valid:true with an empty errors array and errors_total 0; detail=errors on it returns the exact array [].",
        example: None,
    },
    ToolDoc {
