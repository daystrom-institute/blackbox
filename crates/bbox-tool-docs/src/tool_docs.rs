...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        summary: "List visible project graphs in bounded pages (default 20, max 100, also byte-budgeted) ordered by graph_id, source, then checkout. Continue with next_offset plus expected_view_stamp.",
        when_to_use: "Discover graph ids. Pass provisional (published, own, or all); visibility is a deprecated alias. Each entry carries two count families: vertex_count/edge_count count the REFLECTED graph (authored rows plus schema-as-data type vertices and meta:INSTANCE_OF edges), while authored_vertex_count/authored_edge_count count only rows from vertices.jsonl/edges.jsonl; compare authored_* against your source files. source names the authority plane: published, provisional, or connector (read-only connector-managed projection, never writable through a checkout lane). The inventory is a live view, not a snapshot: published installs and provisional overlays replace whole entries, so a changed view_stamp refuses nonzero offsets and you restart at offset 0. Empty inventories return total 0 with an empty graphs array; a missing graph id under describe/validate is error.not_found.",
        example: None,
    },
    ToolDoc {
    ...
    ToolDoc {
        ...
        when_to_use: "Use before an advance to read compare-and-swap tokens, or to diagnose unavailable, prior-fallback, detached, or non-advancing published knowledge. Use detail=health or detail=connector for exact diagnostics; a changed body refuses continuation. detail=auto_advance recovers complete auto-advance policy and attempt evidence. detail_limit and detail_cursor require detail; limits outside 4..4096 are rejected before collection.",
        example: None,
    },
    // ── Knowledge ────────────────────────────────────────────────────
    ToolDoc {
    ...
    ToolDoc {
        ...
        when_to_use: "Explicitly add/remove MCP servers and manage dispatch-time tool filters in the daemon-owned store. scope selects exactly one store: global (default, under BRO_HOME on the daemon host) or project (project is required with scope=project and rejected for global). list(body_limit=4096) pages the exact redacted inventory including full identities; ordinary list pages server rows (default 20, max 100, continue with offset; a past-end offset is an honest empty page, and the next-page hint repeats the required project selector) with bounded name/pattern previews; get_filters (same scope) pages the exact filter inventory when previews truncate - never use a mutating reset as read recovery. get reads the selected store and returns the exact redacted config (name, endpoint origin, header/env key identity, exclude list) as bounded JSON body pages: concatenate body.text using cursor=body.next_cursor, then parse; cursors bind scope, store identity, name, and content. A missing server reply names which store it is absent from. add supports http and sse transports only; stdio is rejected with no add lane. sync is retired: it fails honestly without reading or resolving stored secrets because no dispatch provider consumes persistent provider-CLI registration; dispatched bros receive per-dispatch injection. Replies are redacted (endpoint origins and credential key names visible, values and stdio arguments withheld) and never echo daemon-local store paths. The daemon does not rewrite provider MCP configs. Before `action=add`, call `action=list` first. The default bro-tool disallow is mechanical recursion protection, not just prose guidance. See `sm-create-etiquette` via `bbox_knowledge` for dedupe hygiene. In catalog mode project names a catalog project (id, alias or attached checkout path; it never grants daemon filesystem access) and scope=project goes through the checkout-owner lane. Reads (list, list inventory, get, get_filters) answer from the project's accepted publication of .bbox/mcp.json, with enablement from its committed .bbox/config.toml, and carry projectId, source (accepted generation and commit) and ownerEdits, a byte-budgeted summary (total, omitted, the newest edits with state and next step) of queued, delivered, conflicted or blocked edits not yet reflected; list(body_limit=4096) pages every open edit with its landing path under body owner_edits. add/remove/allow/disallow/clear_filters queue a guarded edit of .bbox/mcp.json for the project's checkout owner and return state=queued with `mutation` (mutation_id, landing path, exact-byte precondition, predecessor, accepted_generation, next_step), or state=unchanged when the edit base already matches; edits before publication chain on the queued ones. An edit takes effect for reads and dispatch only after the owner commits and publishes it. A conflicted edit keeps the owner's bytes: reconcile in the owning checkout, then re-issue the edit, which is recomputed from accepted configuration with a fresh precondition. Named refusals: error.project_config_publication_unavailable, error.project_config_lane_unsupported, error.project_config_invalid, error.project_config_project_unknown, error.checkout_mutation_conflict (publication moved under queued edits) and error.mcp_project_store_too_large; none falls back to the global store. Bridge mode reads and writes the local project store. Retired sync refuses before project resolution in every mode.",
        example: Some(
