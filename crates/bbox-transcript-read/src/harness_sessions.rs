...
/// Reader-side bridge to the tantivy projector (`bbox_corpus_index::
/// transcripts::harness_sessions::project_fleet_event_log`).
///
/// Given the committed prefix contents of a fleet event log, resolve its
/// owning provider + location and normalize every conversational event. The
/// projector turns the returned events into tantivy documents; this crate
/// stays tantivy-free, so the normalization lives here and the doc build stays
/// there. `session_id` must be non-empty and `contents` must be the validated
/// prefix bytes returned by [`read_fleet_event_log_prefix`] as UTF-8.
pub fn normalize_fleet_prefix(
    canonical_path: &Path,
    session_id: &str,
    contents: &str,
) -> Result<(TranscriptLocation, Vec<NormalizedTranscriptEvent>), TranscriptReadError> {
    ...
    let location = adapter.location_for(session_id, canonical_path.to_path_buf(), &meta);
    let batch = adapter.read_contents_since(&location, contents, 0)?;
    Ok((location, batch.events))
}
...
mod tests {
    use super::*;
    use crate::types::TranscriptEventKind;
    use serde_json::json;
    ...
    #[test]
    fn read_fleet_prefix_stops_at_the_acknowledged_sequence() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path().canonicalize().unwrap();
        let path = write_log(
            &root,
        ...
        let text = std::str::from_utf8(&prefix.bytes).unwrap().to_string();
        let (location, events) =
            normalize_fleet_prefix(&prefix.canonical_path, "fleet-prefix", &text).unwrap();
        assert_eq!(location.session_id.as_deref(), Some("fleet-prefix"));
        assert_eq!(events.len(), 1);
        assert!(text.contains("acknowledged"));
        assert!(!text.contains("not acknowledged"));
    }
