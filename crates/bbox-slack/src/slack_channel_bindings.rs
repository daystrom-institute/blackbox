...
/// Stamp one channel binding with its stable project id, the write-back
/// inverse of [`capture_project_catalog_owner_snapshot`]. The backfill's row id
/// is the `<team_id>:<channel_id>` pair the capture emits, not the map key.
...
> {
    ...
    stamp_json_owner_row(
        ...
        limits,
        |bytes| {
            stamp_json_map_row(bytes, "bindings", source_row_id, project_id, |row| {
...
mod owner_row_stamping {
    ...
    const ROW_A: &str = "T1:C1";
    const ROW_B: &str = "T1:C2";
    ...
    fn write_fixture(dir: &tempfile::TempDir) -> std::path::PathBuf {
        ...
        std::fs::write(
            ...
            br#"{
  "bindings": {
    "k1": {"team_id": "T1", "channel_id": "C1", "project_dir": "/legacy/path/one", "future_field": {"kept": true}},
    "k2": {"team_id": "T1", "channel_id": "C2", "project_dir": "/legacy/path/two"}
  }
}
"#,
        )
