...
pub struct KnowledgeEntry {
    ...
    /// Project-scoped entry whose repo-owned file was placed by a satellite
    /// (`bro render` drain + ack) while this daemon could not see the
    /// checkout (remote corpus host, gap-4e2db371). The entry stays central
    /// for recall until the checkout's committed copy reaches this daemon
    /// (e.g. via a connector mount resync); the flag keeps it out of future
    /// render-plan `repo_records` so satellites do not re-place it.
...
impl Knowledge {
    ...
    // machine. These methods compute WHAT a render would write, with zero
    // filesystem access, so the /render/plan endpoint can serve it to a
    // satellite (`bro render`) that owns the local writes.

    /// Project-file plan: `(file name, full contents)` per provider file a
    /// project render would write. `has_project_md` replaces the local
    /// PROJECT.md existence probe (the caller checked its own checkout).
    pub fn render_plan_project(
        &self,
        scope_dir: &str,
        has_project_md: bool,
    ) -> Result<Vec<(String, String)>> {
        let mut plans = Vec::new();
        for prov in ["claude", "agents", "gemini"] {
            let filter = ScopeFilter::Project(scope_dir);
            let mut body = String::new();
            self.render_steerage(prov, filter, &mut body);
            let push_include = |body: &mut String| {
                if has_project_md {
                    body.push_str(project_include_instruction(prov));
                    body.push('\n');
                }
            };
            // Gemini deprioritizes bottom content: include goes between
            // steerage and memory (mirrors render_project_body exactly).
            if prov == "gemini" {
                push_include(&mut body);
                self.render_memory(prov, filter, &mut body);
            } else {
                self.render_memory(prov, filter, &mut body);
                push_include(&mut body);
            }
            ...
            full.push_str(&body);
            plans.push((target_file(prov, Some(scope_dir)), full));
        }
        Ok(plans)
    }

    /// Global managed-region plan: `(target key, managed body)` for the
    /// shared include ("common") and each provider file. `common_path` is
    /// the CALLER's shared-include path (the provider bodies embed a
    /// reference to it, and the daemon's own path would be wrong on a
    /// remote host).
    pub fn render_plan_global(&self, common_path: &Path) -> Result<Vec<(String, String)>> {
        let mut parts = Vec::new();
        parts.push(("common".to_string(), self.render_global_common_body()?));
        for prov in ["claude", "agents", "gemini"] {
            parts.push((prov.to_string(), self.render_global_body(prov, common_path)?));
        }
        Ok(parts)
    }

    /// Project-scoped entries this daemon could NOT place repo-owned (no
    /// visible checkout) that a satellite should drain into the repo:
    /// `(id, relpath, contents)` in the exact committed-file serialization
    /// `persist_repo_kb_entries` uses.
    pub fn repo_pending_records(&self, project: &str) -> Result<Vec<(String, String, String)>> {
        let mut records = Vec::new();
        for e in &self.store.entries {
            if e.project.as_deref() != Some(project) || e.repo_placed {
                continue;
            }
            if self.repo_owned_carrier(e).is_some() {
                continue; // this daemon can place it itself
            }
            let mut on_disk = e.clone();
            on_disk.project = None;
            on_disk.recall_count = 0;
            on_disk.last_recalled = None;
            ...
            records.push((
                e.id.clone(),
                format!(".bbox/knowledge/{}.json", e.id),
    ...
    /// Satellite ack: the listed entries now exist as committed repo files.
    /// They stay central for recall (this daemon still cannot read the
    /// checkout) but leave the pending set. Returns how many were marked.
    pub fn mark_repo_placed(&mut self, project: &str, ids: &[String]) -> usize {
        let now = Self::now_iso();
        let mut marked = 0usize;
        for e in &mut self.store.entries {
            if e.project.as_deref() == Some(project)
                && !e.repo_placed
    ...
    pub fn repo_record_rider(&self, id: &str) -> Option<String> {
        ...
        let Some(dir) = self.repo_owned_carrier(entry) else {
            // Project-scoped but unplaceable from THIS daemon. When the
            // checkout is not even visible (remote corpus host,
            // gap-4e2db371), say so instead of silently keeping the entry
            // central: the satellite drains it with `bro render`.
            let project = entry.project.as_deref()?;
            if !Path::new(project).is_dir() {
                return Some(format!(
                    "\nRepo-owned placement PENDING: this daemon cannot see {project} \
                     (remote corpus host?). Run `bro render --project {project}` on the \
                     machine that owns the checkout to place .bbox/knowledge/{id}.json."
                ));
            }
            return None;
        };
    ...
    pub fn render(&self, p: &RenderParams) -> Result<String> {
        ...
        if do_project {
            ...
            // Fail closed when the checkout is not visible from this daemon
            // (remote corpus host, or a plain wrong path): a silent fallback
            // would either ENOENT mid-write or, worse, present a
            // central-store-only dry-run view as the whole truth
            // (gap-4e2db371).
            if !Path::new(dir).is_dir() {
                anyhow::bail!(
                    "project render refused: {dir} is not a directory on this daemon's filesystem. \
                     If the daemon runs remotely (corpus host), render satellite-side with \
                     `bro render --project {dir}` on the machine that owns the checkout."
                );
            }
...
",
    ...
    #[test]
    fn render_refuses_project_dirs_this_daemon_cannot_see() {
        let central = tempfile::tempdir().unwrap();
        let kb = Knowledge::open(&central.path().join("kb.json")).unwrap();
        let err = kb
            .render(&RenderParams {
                scope: Some("project".into()),
                project: Some("/definitely/not/a/real/checkout".into()),
                dry_run: Some(true),
                ..Default::default()
            })
            .unwrap_err();
        let msg = format!("{err:#}");
        assert!(msg.contains("bro render"), "remedy missing: {msg}");
    }

    #[test]
    fn render_plan_project_matches_written_projection() {
        let _lock = bbox_util::util::test_env_lock();
        let central = tempfile::tempdir().unwrap();
        let repo = tempfile::tempdir().unwrap();
        let repo_root = repo.path().canonicalize().unwrap();
        let proj = repo_root.to_string_lossy().to_string();
        std::fs::write(repo_root.join("PROJECT.md"), "# project\n").unwrap();

        let mut kb = Knowledge::open(&central.path().join("kb.json")).unwrap();
        kb.store.entries.push(entry(
            "planmatch",
            "Plan parity",
            "plan and render must agree",
            Scope::Project,
        ));
        kb.store.entries.last_mut().unwrap().project = Some(proj.clone());

        // The plan (computed with zero fs access, include forced on) must
        // equal the file a local render writes for the same scope.
        let plan = kb.render_plan_project(&proj, true).unwrap();
        ...
        for (file_name, contents) in &plan {
            let written = std::fs::read_to_string(repo_root.join(file_name)).unwrap();
            assert_eq!(&written, contents, "plan/render divergence for {file_name}");
        }
    }
    ...
    fn repo_pending_records_round_trip_through_mark_placed() {
        ...
        // A project path this daemon cannot see: unplaceable → pending.
        kb.store.entries.last_mut().unwrap().project =
            Some("/no/such/checkout/anywhere".into());

        let pending = kb
            .repo_pending_records("/no/such/checkout/anywhere")
            .unwrap();
        assert_eq!(pending.len(), 1);
        let (id, relpath, contents) = &pending[0];
        assert_eq!(id, "pend0001");
        assert_eq!(relpath, ".bbox/knowledge/pend0001.json");
        // Committed-file shape: no project KEY (scope stays "project"),
        // pretty JSON.
        assert!(!contents.contains("\"project\":"));
        assert!(contents.contains("\"id\": \"pend0001\""));

        assert_eq!(
            kb.mark_repo_placed("/no/such/checkout/anywhere", &[id.clone()]),
            1
        );
        assert!(
            kb.repo_pending_records("/no/such/checkout/anywhere")
                .unwrap()
                .is_empty(),
            "placed records must leave the pending set"
        );
        // Still present centrally for recall.
        assert!(kb.entry("pend0001").is_some());
    }
}
