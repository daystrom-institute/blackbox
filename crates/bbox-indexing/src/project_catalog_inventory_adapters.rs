...
use crate::project_catalog_migration::ProjectCatalogMigrationError;
...
// ---------------------------------------------------------------------------
// Durable row stamping dispatch
// ---------------------------------------------------------------------------

/// The production [`LegacyRowStamperV1`]: the write-back counterpart of
/// [`capture_durable_owner_snapshots`].
///
/// It lives beside the capture dispatch deliberately. The two fan out across
/// the same 14-owner set and must agree on the stable row ids the backfill
/// ledger keys on, so a change to one is a change to the other; separating
/// them would let the read and write halves drift apart silently. Its
/// dependency shape is consequently identical to capture's: this crate already
/// depends on every owner crate the stamper writes, and adds no new edge.
pub(crate) struct ProjectCatalogOwnerRowStamperV1 {
    paths: ProjectCatalogOwnerInventoryPathsV1,
    limits: OwnerSnapshotLimitsV1,
}
...
impl ProjectCatalogOwnerRowStamperV1 {
    pub(crate) fn new(
        ...
        // Authorize once up front so an unsafe or malformed owner path fails
        // the whole backfill immediately rather than on the row that happens
        // to touch it. The proof is re-derived per write below; this call is
        // the fail-fast, not the guarantee.
        authorize_owner_paths(paths.clone())?;
        Ok(Self { paths, limits })
    }

    /// Re-prove the path is safe, then write.
    ///
    /// The authority proof is rebuilt for EVERY stamp instead of being cached
    /// from construction, and that is load-bearing rather than defensive.
    /// [`AuthorizedInventoryPath`] pins the target's inode identity, which is
    /// exactly right for a read-only capture ("nothing changed underneath
    /// me") and exactly wrong for a stamping pass: an atomic replace mints a
    /// new inode, so a cached proof would reject THIS stamper's own preceding
    /// write and a multi-row pass over one store would fail on its second row.
    /// Rebuilding keeps the property that actually matters at a write - the
    /// path is absolute, symlink-free, and rooted in a nofollow-opened real
    /// directory - and re-establishes it immediately before each mutation.
    fn stamp_owner_path(
        &self,
        path: &Path,
        stamp: impl FnOnce(
        ...
        let authorized =
            AuthorizedInventoryPath::new(path).map_err(|error| stamp_refusal(error.to_string()))?;
        map_stamp_outcome(stamp(authorized.as_path()))
    }
}
...
fn map_stamp_outcome(
    ...
    use bbox_corpus_core::project_catalog_snapshot::OwnerRowStampOutcomeV1;
...
mod tests {
    ...
    fn attachment_discovery_is_stable_and_duplicate_roots_refuse() {
        ...
        .unwrap_err();
        assert_eq!(error.code, "error.project_catalog_inventory_adapter_input");
    }
}

// ── Durable row stamping dispatch (P6-B) ────────────────────────────────────
    ...
    fn stamper(root: &Path) -> ProjectCatalogOwnerRowStamperV1 {
        ...
        ProjectCatalogOwnerRowStamperV1::new(
            ProjectCatalogOwnerInventoryPathsV1 {
                corpus_index_root: root.join("corpus"),
                git_cursor_root: root.join("cursors"),
                vector_root: root.join("vectors"),
                ...
                provenance_notes_ref: "refs/notes/bbox/provenance".to_string(),
            },
