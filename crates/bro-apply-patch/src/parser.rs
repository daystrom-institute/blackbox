...
// Vendored from openai/codex `codex-rs/apply-patch/src/parser.rs` (Apache-2.0).
// Local change: dropped `Hunk::resolve_path` and the `AbsolutePathBuf` dep —
// path resolution against the worktree root lives in `apply.rs`. See NOTICE.
