...
/// Read a user-supplied `--args-file` at CLI startup. Blocking fs is denied by
/// the workspace clippy gate except at sanctioned sites; this is the startup
/// read of a small, user-provided file.
#[allow(clippy::disallowed_methods)]
fn read_args_file(path: &Path) -> Result<String> {
...
async fn main() -> Result<()> {
    ...
    let cx = make_cx(root);
    match tool.call(input, &cx).await {
        ToolResult::Json(v) => {
            if let Some(pointer) = &cli.field {
