    ...
    async fn call(&self, input: Value, _cx: &ToolCx) -> ToolResult {
        ...
        match transform {
            ...
            other => ToolResult::Error(format!(
                "rust.describe: unknown transform `{other}` (available: fixRound, extractItems, inlineModToFile, moduleWiring, setVisibility, extractImplMethods, organizeImports, moveStructFields, migrateErrorType, migrateTypeUsages, updateCallers, rewriteModuleCallers)"
            )),
        }
    }
}
