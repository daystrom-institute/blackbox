...
impl BindingToolSession {
    pub fn new() -> Self {
        ...
        // The same ledger Arc is shared across lsp.* (producer), rust.fixRound
        // (producer, compiler_suggested tier), and edits.* (consumer) so
        // issuance records survive to the apply choke point. Clone before
        // lsp_facts consumes the original.
        let ledger_for_rust = Arc::clone(&ledger);
        ...
        tools.extend(rust_transforms::tools(ledger_for_rust));
