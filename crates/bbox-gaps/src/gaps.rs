...
/// Read one central gap row's stable project id, the VERIFY half of
...
> {
    ...
    read_json_owner_row(store_path, "gap", "gap:central-json", limits, |bytes| {
        read_json_array_row_project_id(bytes, "gaps", "id", source_row_id)
    })
}
