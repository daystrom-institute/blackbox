...
/// Read one central note row's stable project id, the VERIFY half of
...
> {
    ...
    read_json_owner_row(store_path, "note", "note:central-json", limits, |bytes| {
        read_json_array_row_project_id(bytes, "notes", "id", source_row_id)
    })
}
