...
- The current checkout routes cockpit workers to `surface=interactive`, which preserves `bbox_render`, while workflow workers remain on `agent-internal` and recursive agent/atom dispatches remain on `default`. This follow-up is not considered live until the exact commit passes verification, is deployed by immutable digest, and the bounded three-view render succeeds without a daemon checkout observation or pod restart.
