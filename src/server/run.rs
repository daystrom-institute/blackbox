...
fn init_logging(home: &Path, migrated: Vec<String>) {
...
fn discover_transcript_roots(cfg: &config::Config, home: &Path) -> Vec<(String, PathBuf)> {
...
fn resolve_codex_root(cfg: &config::Config, home: &Path) -> Option<PathBuf> {
    cfg.transcripts
        .codex_root
        .clone()
        .map(|p| {
...
fn configure_dispatch_mcp_env(cfg: &config::Config) {
