...
| `java-extract-interface` | shipped (v1, RA-X3) | normal | `extract_java_interface` + optional `migrate_java_type_usages` | Extract interface from class (signatures + implements + visibility widening) with optional caller migration; operator-authority `acknowledge_public_api_change` is prompt-discipline-only until JAVA_GAP.md Gap 1 (`java_public_api_guard`) ships |
