...
pub struct RefactorPlanParams {
    ...
    /// `extract_java_class`: control whether class-level annotations on
    /// the source class propagate to the extracted target (G13). Values:
    /// `auto` (default) — propagate when moved bodies reference an
    /// identifier generated by the annotation processor (`@Slf4j` →
    /// `log` is the supported v1 case);
    /// `all` — copy every class-level annotation verbatim;
    /// `none` — current behavior (strip every annotation on target);
    /// `list:@A,@B` — operator-supplied allowlist of annotations to
    /// propagate.
    #[serde(default)]
...
fn plan_dispatch(p: &RefactorPlanParams, ctx: &PlanContext) -> Result<String> {
    match p.kind.as_str() {
        ...
        "extract_rust_section" => plan_extract_rust_section(p),
        "move_rust_items_with_local_deps" => plan_move_rust_items_with_local_deps(p),
        ...
        "rust_module_wiring" => plan_rust_module_wiring(p),
        ...
        "inline_java_method" => plan_inline_java_method(p),
        "extract_java_test_slice" => plan_extract_java_test_slice(p),
        "java_collapse_call_chain" => plan_java_collapse_call_chain(p),
        "migrate_java_method_receiver" => plan_migrate_java_method_receiver(p),
        "java_split_provider" => plan_java_split_provider(p),
        "replace_java_static_reference" => plan_replace_java_static_reference(p),
        "singletonify_java_holder" => plan_singletonify_java_holder(p),
        "singletonify_java_util" => plan_singletonify_java_util(p),
...
pub fn run_with_ctx(
    ...
    let workspace_adapter = workspace_adapter::resolve_workspace_adapter(&steps, &project_dir)?;
                    ...
                    let plan_json =
                        match rust_compile_fix::plan_compile_fix(&step_params, &diagnostics) {
                            ...
                            Err(err) => {
                                ...
                                let rollback_errors = rollback_run(
                                    &snapshots,
                                    cursor,
                                    workspace_adapter.as_ref(),
                                    &run_id,
                                );
...
pub fn parse_dotnet_build_output(stdout: &[u8]) -> Vec<DotnetBuildDiagnostic> {
    ...
    for raw_line in text.lines() {
        ...
        let (file, line_no) = match (prefix.rfind('('), prefix.rfind(')')) {
            ...
            _ => (
                Some(prefix.trim().to_string()).filter(|s| !s.is_empty()),
                None,
            ),
        };
