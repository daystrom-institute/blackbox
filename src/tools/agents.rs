...
impl BlackboxServer {
    ...
    #[tool(
        ...
        description = "Dispatch a registered agent for a focused task. Resolves the brofile, merges filters, validates inputs and starts one bro turn. Custom dispatch adapters are retired and refused. Returns task_id, session, and agent attribution (agentLabel on the spawned task, preserved even when bro= routes to a named team member)."
    )]
