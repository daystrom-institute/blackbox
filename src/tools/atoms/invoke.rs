...
impl BlackboxServer {
    ...
    pub(crate) async fn atom_invoke_value(
        ...
        match &manifest.implementation {
            AtomImplementation::Consultant { .. } => return Err(
                "error.retired(code=consultant_runtime_removed): consultant execution is retired; use bro_exec or an ordinary named bro agent".into()
            ),
            AtomImplementation::Adapter { adapter_name } if adapter_name == "badgey" => return Err(
                "error.retired(code=badgey_adapter_removed): Badgey execution is retired; use bro_exec or an ordinary named bro agent".into()
            ),
            _ => {}
        }
        ...
        match &manifest.implementation {
            ...
            AtomImplementation::Consultant { .. } => Err(
                "error.retired(code=consultant_runtime_removed): consultant execution is retired; use bro_exec or an ordinary named bro agent".into()
            ),
        }
    }
    ...
    async fn atom_invoke_profile(
        ...
        let dispatched = self
            .dispatch_fresh_bro_task(crate::tools::dispatch::FreshDispatchRequest {
                ...
                disallow_tools: None,
                tool_placement: None,
                brofile_tool_defaults: bf.tool_defaults,
                tool_defaults: None,
