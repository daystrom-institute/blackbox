...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use after `sm-refactor` identifies an exact supported plan kind. For syntax-item operations, inspect with bbox_refactor_status first and use the relevant language memory for exact item kinds and extra arguments. Supported Java extraction/wiring plans include extract_java_methods, extract_java_class, add_java_fields, add_java_constructor, move_java_field, move_java_constant, add_java_delegate_field, and update_java_callers; use them instead of write_file/replace_text for the normal extract-class follow-through. The `lombokify_java_class` plan replaces hand-rolled POJO boilerplate (trivial getters/setters, Apache Commons builder-style equals/hashCode/toString, canonical `@*ArgsConstructor`-shape constructors, SLF4J `Logger log` field) with Lombok annotations of equivalent semantics; full-coverage classes collapse to `@Data` or `@Value`. Single-file or directory-tree (`source` is a directory) modes; bulk mode aggregates per-file FileEdits into one plan and reports skipped files via `leftovers`. Pass `boolean_getter_strategy=bridge` to emit a `getXxx()` bridge after `@Getter` for `boolean` fields whose original getter used the `get-` prefix (Lombok generates `isXxx()`); default `skip` preserves API by leaving the field's getter alone. Pass `output_path=<file>` to write the full plan JSON to disk and receive a compact summary inline — required for plans whose body exceeds the MCP transport's parameter-string limit. Apply via `plan_path` with the same path. Lombok must already be on the project's classpath. Pass `deep_analysis: true` on `move_java_field`, `extract_java_methods`, or `extract_java_class` to receive pessimistic safety reports (default false keeps the response lean for self-contained clusters): `move_java_field` adds `remaining_source_accessors` listing every read/write of each moved field that still lives in the source class (line, column, kind, context) — a non-empty `accesses` array flags lines that will fail to compile until you rewrite or move them. `extract_java_methods` / `extract_java_class` always return `captured_variables` (source-class fields used by extracted methods); with `deep_analysis: true` they additionally return `external_calls` (source-class methods called by the extracted set but not in it, with `signature` and `call_sites[].context = direct|lambda`) and `inherited_dependencies` (superclass/interface methods reached via the project type index, with `source` and `source_kind`). With `deep_analysis: true` AND `move_fields` non-empty, `extract_java_class` also rewrites every remaining source-side read/write of a moved field through the delegate's getter/setter and emits matching accessor declarations on the target (defaults on; pass `rewrite_remaining_accessors: false` to opt out and keep only the `remaining_source_accessors` report). Resolve every reported entry before applying so the target class doesn't ship with unresolved references. Supported extraction plans may create a missing target as an empty-original FileEdit; do not pre-create placeholder files or use allow_dirty_worktree=true for normal target creation. Java import repair uses java_lsp_organize_imports, preferring a warm per-project JDTLS session (lazy-spawned and reused across calls) and falling back to source-tree heuristics that preserve qualified inner-class references. Generic plans cover file moves, exact text replacement, whole-file writes, and simple TOML table ensures; exact text replacement is not semantic rename. The plan is reviewable JSON with hash checks, file moves or text edits, parse validations, selected items, and leftovers where applicable. Keep this generic surface free of language-specific assumptions; semantic rename belongs to LSP, compiler validation, or language-scoped surfaces.",
