...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use after `sm-refactor` identifies an exact supported plan kind. For syntax-item operations, inspect with bbox_refactor_status first and use the relevant language memory for exact item kinds and extra arguments. Supported Java extraction/wiring plans include extract_java_methods, extract_java_class, add_java_fields, add_java_constructor, move_java_field, add_java_delegate_field, and update_java_callers; use them instead of write_file/replace_text for the normal extract-class follow-through. Supported extraction plans may create a missing target as an empty-original FileEdit; do not pre-create placeholder files or use allow_dirty_worktree=true for normal target creation. Java import repair uses java_lsp_organize_imports, preferring JDTLS and falling back to source-tree heuristics. `extract_java_methods` and `extract_java_class` plans return `captured_variables` (source-class fields used by extracted methods), `external_calls` (source-class methods called by the extracted set but not in it, with `signature` and `call_sites[].context = direct|lambda`), and `inherited_dependencies` (superclass/interface methods reached via the project type index, with `source` and `source_kind`); resolve every entry before applying so the target class doesn't ship with unresolved references. Generic plans cover file moves, exact text replacement, whole-file writes, and simple TOML table ensures; exact text replacement is not semantic rename. The plan is reviewable JSON with hash checks, file moves or text edits, parse validations, selected items, and leftovers where applicable. Keep this generic surface free of language-specific assumptions; semantic rename belongs to LSP, compiler validation, or language-scoped surfaces.",
