...
pub const TOOL_DOCS: &[ToolDoc] = &[
    ...
    ToolDoc {
        ...
        when_to_use: "Use after `sm-refactor` identifies an exact supported plan kind. For syntax-item operations, inspect with bbox_refactor_status first and use the relevant language memory for exact item kinds and extra arguments. Supported Java extraction/wiring plans include extract_java_methods, extract_java_class, add_java_fields, add_java_constructor, move_java_field, add_java_delegate_field, and update_java_callers; use them instead of write_file/replace_text for the normal extract-class follow-through. Supported extraction plans may create a missing target as an empty-original FileEdit; do not pre-create placeholder files or use allow_dirty_worktree=true for normal target creation. Java import repair uses java_lsp_organize_imports, preferring a warm per-project JDTLS session (lazy-spawned and reused across calls) and falling back to source-tree heuristics that preserve qualified inner-class references. Generic plans cover file moves, exact text replacement, whole-file writes, and simple TOML table ensures; exact text replacement is not semantic rename. The plan is reviewable JSON with hash checks, file moves or text edits, parse validations, selected items, and leftovers where applicable. Keep this generic surface free of language-specific assumptions; semantic rename belongs to LSP, compiler validation, or language-scoped surfaces.",
